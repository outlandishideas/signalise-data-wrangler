...
SELECT location__location, COUNT(nacs_code__text4)
FROM monday.ccg_framework_locations
GROUP BY location__location
ORDER BY count DESC;
...
SELECT LPAD(EXTRACT(HOUR FROM start__hour)::TEXT, 2, '0') || ':' ||
     ...
     , met_sla_booking_time(zip_date_hour(booking_date__date, start__hour), deal_closed__date_confirmed8,
                            deal_closed__date_confirmed8)                              AS confirmed_within_appropriate_level_timescale
...
SELECT (booking_date__date::DATE || ' ' || LPAD(EXTRACT(HOUR FROM start__hour)::TEXT, 2, '0') || ':' ||
        LPAD(EXTRACT(MINUTES FROM start__hour)::text, 2, '0'))::timestamp              AS  booking_start_datetime
     , (deal_closed__date_confirmed8::DATE || ' ' ||
        LPAD(EXTRACT(HOUR FROM deal_closed__date_confirmed8)::TEXT, 2, '0') || ':' ||
        LPAD(EXTRACT(MINUTES FROM deal_closed__date_confirmed8)::text, 2, '0'))::TIMESTAMP deal_closed_datetime
